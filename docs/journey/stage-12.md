# Stage 12 · Pub/Sub and supervised shutdown
...
??? note "File diff: tests/concurrency/test_async_invariants.py"
    ```diff
...
??? note "File diff: tests/concurrency/test_shutdown.py"
    ```diff
...
??? note "File diff: tests/concurrency/test_slow_endpoint.py"
    ```diff
...
??? note "File diff: tests/mechanisms/test_pubsub.py"
    ```diff
...
#### Typed Pub/Sub commands

Preserve exact binary channels and explicit subscribe, unsubscribe, and publish intent at the parser boundary.
...
#### Bidirectional subscription ownership

Index channels by session and sessions by channel so publish and close cleanup are both bounded.
...
#### Serialized Pub/Sub and shutdown barrier

Deliver push output and terminalize waiters, requests, subscriptions, and endpoints in executor order.
...
#### Supervised maintenance and shutdown

Quiesce control producers, bypass full user admission with a shutdown control, drain endpoints briefly, and release every owned task.
...
#### Idempotent Direct session close

Turn client close into a shielded executor control and map lifecycle outcomes without bypassing session ownership.
...
[Compare this stage on GitHub](https://github.com/system-in-miniature/mini-redis/compare/bb842dd...6ff1e5f)

After finishing, run `python -m journey.tools.build_journey check 12` to verify the learner workspace.
