...
??? note "文件差异：tests/reliability/test_ambiguous_outcome.py"
    ```diff
...
??? note "文件差异：tests/reliability/test_reliability_shutdown.py"
    ```diff
...
??? note "文件差异：tests/reliability/test_worker_failure.py"
    ```diff
...
把已接纳的 Replica Apply 排在 Promotion 之前，作废旧 Source Generation，并只在屏障之后重新开放写入。
...
Replica 只读期间保持 Control Admission 存活，并在 Runtime 成为新 Primary 时重新开放 User Admission。
...
区分 Graceful Close 与 Crash Close，并在文件描述符消失前收敛正在进行的 Sync 工作。
...
#### Recovery-first 受监督生命周期

在接纳请求前完成恢复，协调 Snapshot、AOF、Replica 与 Executor 的关闭，并在 Worker 死亡时终结所有待处理所有权。
...
[在 GitHub 查看阶段差异](https://github.com/system-in-miniature/mini-redis/compare/e18be82...0fbaeee)

完成后可运行 `python -m journey.tools.build_journey check 18` 验收学习工作区。
