# Stage 19 · RESP2 protocol boundary
...
??? note "File diff: tests/adapters/test_resp2_decode.py"
    ```diff
...
??? note "File diff: tests/adapters/test_resp2_encode.py"
    ```diff
...
#### Binary-safe RESP2 boundary

Incrementally decode bounded wire frames, keep bulk values as bytes, map command arrays into domain requests, and encode every outbound value explicitly.
...
[Compare this stage on GitHub](https://github.com/system-in-miniature/mini-redis/compare/0fbaeee...c088652)

After finishing, run `python -m journey.tools.build_journey check 19` to verify the learner workspace.
