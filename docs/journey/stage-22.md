...
??? note "File diff: tests/adapters/test_direct_pipeline.py"
    ```diff
...
#### Token-ordered pipeline completion

Register every admitted request, buffer out-of-order completions, and flush replies only from the oldest completed prefix.
...
Submit all queued requests before awaiting results, preserve one result slot per input, and make parse failures enter mailbox order.
...
#### Bounded TCP pipeline admission

Admit multiple decoded frames without serial reply waits, retry temporary BUSY admission, and settle every command/admission task on close.
...
Expose DirectPipeline through the package while keeping the normal direct-client semantics as its reusable primitive.
...
[Compare this stage on GitHub](https://github.com/system-in-miniature/mini-redis/compare/40d00de...0016059)

After finishing, run `python -m journey.tools.build_journey check 22` to verify the learner workspace.
