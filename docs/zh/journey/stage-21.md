...
建模并解析 MGET、MSET 与 DECR，再规划有序读取以及一次 Commit、重复 Key 归一化的写入。
...
用一个类型化方向 Flag 统一 BLPOP 与 BRPOP，并让该选择贯穿立即执行和延迟唤醒。
...
暴露 Adapter、Durability、Replication 与 Runtime 的所有权计数，使全系统验收测试能够证明零泄漏。
...
调整完整 Runtime 验收契约使用的测试配置，但不把 Project Metadata 当成 Runtime Mechanism。
...
[在 GitHub 查看阶段差异](https://github.com/system-in-miniature/mini-redis/compare/5419f99...40d00de)

完成后可运行 `python -m journey.tools.build_journey check 21` 验收学习工作区。
