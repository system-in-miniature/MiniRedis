...
#### Primary 持有的物理 Expiry

让 Replica 逻辑隐藏过期 Value，但抑制 Lazy/Active Expiry Commit，使只有 Primary-propagated Delete 推进复制 History。
...
Full Install 或 Partial Resume 后 Quiesce Active-expire Producer，只在成功 Promotion 后重新启动。
...
[在 GitHub 查看阶段差异](https://github.com/system-in-miniature/mini-redis/compare/c07182f...94109b0)

完成后可运行 `python -m journey.tools.build_journey check 29` 验收学习工作区。
