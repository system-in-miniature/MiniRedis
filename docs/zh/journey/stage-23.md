...
??? note "文件差异：tests/contract/test_atomic_functions.py"
    ```diff
...
??? note "文件差异：tests/reliability/test_transaction_commit.py"
    ```diff
...
建模并解析 MULTI、EXEC、DISCARD、WATCH、UNWATCH 与两个原子函数，并加入显式 Null-array Abort Reply。
...
保存 Per-session Queue/Watch State，在深拷贝 Database Fork 上执行排队命令，再发布一个合并 Commit。
...
#### 单 Turn EXEC 与 WATCH Fencing

路由事务控制，遇到 Dirty 或 Revision 变化时 Abort，只保留一次 Blocked Waiter，并收敛为一个 Durable Commit 与有序结果槽。
...
把 Compare-and-delete 与 Checked-decrement 实现为普通 Pure Plan，使独立执行与事务执行共享精确语义。
...
[在 GitHub 查看阶段差异](https://github.com/system-in-miniature/mini-redis/compare/0016059...b195a43)

完成后可运行 `python -m journey.tools.build_journey check 23` 验收学习工作区。
