...
Full Snapshot 安装时写入 Source Identity，Partial Resume 前校验 Identity 与精确 Applied Sequence，并在 Promotion 时生成新 Identity、清空 Backlog。
...
#### Catch-up 先于 Live Streaming

Disconnect 后保留 Cursor，先应用 Frozen Backlog Suffix 再应用并发排队 Live Batch，检测 Sequence Gap，并在 Overflow 或 Source Loss 后重新 Attach。
...
暴露 Retained Backlog Count 与 Oldest Sequence，使测试与 Operator 能解释 Partial 与 Full Fallback。
...
[在 GitHub 查看阶段差异](https://github.com/system-in-miniature/mini-redis/compare/e65b568...c07182f)

完成后可运行 `python -m journey.tools.build_journey check 28` 验收学习工作区。
