...
??? note "文件差异：tests/reliability/test_aof_rewrite.py"
    ```diff
...
#### 原子 Online AOF Rewrite

在 Append Path 外写 State-base Temp File，按 Writer Order 捕获每个并发 Committed Delta，再 Fsync、Rename、Fsync Parent 并切换 Descriptor。
...
#### 无 Capture Gap 的 Executor Registration

在一个 Executor Control Turn 中 Capture Checkpoint 并注册 Rewrite Delta Collection，之后 Commit 才能插入。
...
校验 Delta Bound，暴露一个 Runtime Rewrite Operation 与 Stats，并区分 Graceful Join 和 Crash Abort。
...
[在 GitHub 查看阶段差异](https://github.com/system-in-miniature/mini-redis/compare/b9b363e...8cd6d5e)

完成后可运行 `python -m journey.tools.build_journey check 26` 验收学习工作区。
